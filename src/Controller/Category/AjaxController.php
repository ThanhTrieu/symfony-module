...
class AjaxController extends BaseController
{
    public function getCateStreamPosts(Request $request, Category $cateService, DataExchange $exchangeService, CryptUtils $cryptUtils)
    {
        ...
        $cateStreamCacheKey = sprintf(Constants::CACHE_CATEGORY_LASTEST_TIMESTAMP, $cateId, $curLastInfo['lastPublishedTimestamp'], $curLastInfo['lastPostId']);
        if (($catePostInfo = $cacheService->get($cateStreamCacheKey)) === false) {
            ...
            $sourcePosts = $em->getRepository(PostPublishes::class)->getCatePosts($cateIdList, Constants::PAGE_SIZE + 1, $curLastInfo['lastPublishedTimestamp'], [$curLastInfo['lastPostId']]);
            if ($sourcePosts) {
                $showViewMore = false;
                ...
                $sourcePosts = $exchangeService->ExchangeArrayArticle($sourcePosts, Constants::POST_AVATAR_LIST_SIZE, null, Constants::CUT_NUMBER_CHARACTERS_DEFAULT, Constants::CUT_NUMBER_CHARACTERS_LIST_SUMMARY);
