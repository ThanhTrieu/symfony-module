...
//            throw $this->createNotFoundException('The Tag does not exist');
        ...
        // Get post in tag
        $tagPostsKeyCache = $this->formatCacheKey(Constants::CACHE_FOCUS_CATE, $tagId);
        ...
        $tagRssUrl = str_replace('.html', '/feed.html', $tagUrl);
        $seo = $this->buildPagingMeta($tagUrl, $tag['name'], $tag['description'], $tagRssUrl);
        
        if ($tag['isSpecial']) {
            $data = $dataExchangeService->exchangeTagsIsSpecial($tag,Constants::MOBILE_FIRST_TOP_FEATURED_SIZE);
